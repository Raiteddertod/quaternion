...
def run_simulation(initial_roll=30.0, initial_pitch=15.0, initial_yaw=0.0,
    ...
    from quaternion.utils import angle_between_vectors   # ← важно!
    ...
    for i in range(int(T / dt)):
        ...
        logs['e_z_B'].append(e_z_B)
        logs['e_z_des_B'].append(e_z_des_B.copy())
        logs['n'].append(n)
        logs['B_n'].append(B_n)
        logs['alpha'].append(np.degrees(alpha))   # ← сохраняем
...
def animate_orientation_with_controls(logs, fps=25):
    ...
    def update_display(frame):
        ...
        draw_frame(ax, R_cur, color='blue', label='Текущая СК (синяя Z = тяга)', alpha=0.95, linewidth=2.3)
        ...
        # alpha (с защитой)
        alpha_deg = logs.get('alpha', [0.0] * n_frames)[frame]

        ax.quiver(0, 0, 0, *e_z_des_B, color='orange', linewidth=2.8,
                  arrow_length_ratio=0.1, label='e_z,des_B')

        if np.linalg.norm(n) > 0.03:
            ax.quiver(0, 0, 0, *n, color='red', linewidth=2.2,
                      arrow_length_ratio=0.15, label='n')

        if np.linalg.norm(B_n) > 0.03:
            ax.quiver(0, 0, 0, *B_n, color='purple', linewidth=2.2, linestyle='--',
                      arrow_length_ratio=0.15, label='B_n')
        ...
        legend_text = (
            ...
            "                 (направление тяги)\n"
            "● e_z,des_B    — желаемая ось Z корпуса\n"
            "● n            — ось вращения (мировая СК)\n"
            "● B_n          — ось вращения (связанная СК)"
        )
    ...
    def step():
        ...
        if frame_idx[0] >= n_frames: frame_idx[0] = 0
        elif frame_idx[0] < 0:       frame_idx[0] = n_frames - 1
